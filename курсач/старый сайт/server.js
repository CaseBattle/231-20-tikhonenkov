    ...
    db.run(
      ...
      function (insertErr) {
        if (insertErr) {
          ...
          db.run(
            `INSERT INTO User (email, name, phone, type)
             VALUES (?, ?, ?, ?)`,
            [email, name, phone, 'client'],
            function (fallbackErr) {
              if (fallbackErr) {
                console.error('Ошибка регистрации (fallback):', fallbackErr.message);
                return res.status(500).json({
                  success: false,
...
app.post('/request', (req, res) => {
  const { name, phone, email, comment, propertyId } = req.body;
  ...
  db.run(
    `INSERT INTO Request (property_id, user_name, phone, email, comment, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [
      propertyId || null,
      name,
...
app.get('/admin/requests', requireAdmin, (req, res) => {
  db.all(`SELECT * FROM Request ORDER BY id DESC`, [], (err, rows) => {
    if (err) {
...
app.delete('/admin/requests/:id', requireAdmin, (req, res) => {
  const { id } = req.params;
